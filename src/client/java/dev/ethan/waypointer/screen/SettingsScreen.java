...
import dev.ethan.waypointer.screen.settings.PerfStressTestController;
import dev.ethan.waypointer.screen.settings.RecentSettings;
import dev.ethan.waypointer.screen.settings.Setting;
...
public final class SettingsScreen extends Screen {
    ...
    private void addSearchBox(Layout layout) {
        ...
        searchClearButton = Button.builder(Component.literal("Clear"), b -> clearSearch())
                .bounds(x + searchW + GAP_TIGHT, layout.top() + 4, SEARCH_CLEAR_W, BTN_H)
    ...
    private void buildPerfTestControls(Row row, Setting setting, int controlRight, int rowTop) {
        ...
        Button copy = Button.builder(Component.literal("Copy report"), b -> {
                ...
                .bounds(controlRight - buttonW, 0, buttonW, BTN_H)
                .tooltip(Tooltip.create(Component.literal(
                        "Copy the full performance rundown to your clipboard.")))
                .build();

        boolean running = PerfStressTestController.running();
        Button run = Button.builder(Component.literal(running ? "Cancel test" : "Run test"), b -> {
                ...
                .bounds(controlRight - buttonW * 2 - GAP, 0, buttonW, BTN_H)
    ...
    private void buildConfigCodeControls(Row row, Setting setting, int controlRight, int rowTop) {
        ...
        Button importButton = Button.builder(Component.literal("Import config code"), this::importConfigCode)
                ...
                .build();
        Button copyButton = Button.builder(Component.literal("Copy config code"), this::copyConfigCode)
                .bounds(controlRight - buttonW * 2 - GAP, 0, buttonW, BTN_H)
                .tooltip(tooltipOrNull(setting))
                .build();
    ...
    private void buildPresetControls(Row row, Setting setting, int controlRight, int rowTop) {
        ...
        for (String[] preset : presets) {
            ...
            x -= w;
            Button button = Button.builder(Component.literal(label), b -> applyPreset(id))
                    .bounds(x, 0, w, BTN_H)
                    .build();
    ...
    private void buildDialogActionControl(Row row, Setting setting, int controlRight, int rowTop,
        ...
        Button button = Button.builder(Component.literal(label), b -> onConfirmFlow.run())
                .bounds(controlRight - ENUM_BUTTON_W, 0, ENUM_BUTTON_W, BTN_H)
                .tooltip(tooltipOrNull(setting))
                .build();
        registerRowWidget(row, button, rowTop + 2);
    }
    ...
    private void buildConfirmActionControl(Row row, Setting setting, int controlRight, int rowTop,
        ...
        Button button = Button.builder(Component.literal(current), b -> {
    ...
    private void buildBoolControl(Row row, Setting setting, int controlRight, int rowTop) {
        Checkbox checkbox = Checkbox.builder(Component.empty(), font)
                .pos(0, 0)
                .selected(Boolean.TRUE.equals(setting.get(config, dungeonConfig)))
                .onValueChange((b, v) -> applySetting(setting, v))
                .build();
        checkbox.setX(controlRight - checkbox.getWidth());
        checkbox.setTooltip(tooltipOrNull(setting));
