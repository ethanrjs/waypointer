...
Implemented kinds are 0 (general route), 1 (compact single route), 2 (bare route), 3 (configuration), 4 (dungeon collection), 5 (sparse route), 6 subtype 0 (bare route pack), and 6 subtype 1 (route library). Kind 7 is reserved. Other kind-4 and kind-6 subtypes are reserved.
